...
def test_user_change_password(app: FastAPI, client: TestClient, tester: User, tester_token_headers):
    url = app.url_path_for("user:change-password")
    response = client.post(url, json={"password": "!QAZxsw2"}, headers=tester_token_headers)
    assert response.status_code == 200


def test_user_get_me(app: FastAPI, client: TestClient, tester: User, tester_token_headers):
...
def test_user_put_me(app: FastAPI, client: TestClient, tester: User, tester_token_headers):
    url = app.url_path_for("user:me")
    new_name = "tester01"
    response = client.put(url, headers=tester_token_headers, json={"nickname": new_name})
    assert response.status_code == 200
    resp = Resp[SimpleUser].model_validate(response.json())
    user = cast(SimpleUser, resp.data)
    assert user.email == tester.email
